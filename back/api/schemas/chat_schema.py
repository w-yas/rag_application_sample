...
class RagChatRequest(BaseModel):
    query: Annotated[str, Field(..., description="ユーザーからの問い合わせ内容")]
    top_k: Annotated[
        int, Field(default=3, ge=1, le=50, description="取得するドキュメントの上位K件数")
    ]
    search_mode: Annotated[
        SearchMode, Field(default=SearchMode.FULL, description="サーチモードの指定")
    ]
...
class ChatMessage(BaseModel):
    id: Annotated[
        str, Field(default_factory=lambda: str(uuid4()), description="メッセージの一意な識別子")
    ]
...
class ChatThread(BaseModel):
    ...
    user_id: Annotated[
        str, Field(..., description="チャットスレッドの所有者であるユーザーの識別子")
    ]
    title: Annotated[str, Field(..., description="チャットスレッドのタイトル")]
    created_at: datetime = Field(
        default_factory=datetime.now, description="チャットスレッドの作成日時"
    )
    messages: Annotated[
        list[ChatMessage],
...
class ThreadResponse(BaseModel):
    threads: Annotated[
        list[ChatThread],
        Field(..., description="ユーザーに関連するチャットスレッドのリスト"),
    ]
